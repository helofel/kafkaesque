...
struct node * new_node(char * first_name, char * last_name, int age, char * gender, int id)
{
    struct node *p;
...
struct node * insert(struct node * root, struct node * node)
{
    ...
    if(root == NULL)
        return new_node(node -> data -> first_name, node -> data -> last_name, node -> data -> age, node -> data -> gender, node -> data -> system_id);
